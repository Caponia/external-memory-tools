...
推荐把空白工具做成 Codex Plugin，并通过 GitHub Marketplace 仓库发布。接收者只需添加仓库并在 `/plugins` 中安装，不需要向你索取 ZIP。

ZIP 只作为离线备用方式。无论用哪种方式，都不要分享你已经填写过的 `Memory`、`INBOX`、`Exports` 或私人工作区。
...
推荐方式：把空白 Skill 包装成 Codex Plugin，放进独立的 GitHub Marketplace 仓库。不要把正在使用的记忆根目录放进这个仓库。
...
然后进入 Codex CLI，输入 `/plugins`，在 **External Memory Tools** Marketplace 中安装 **External Memory Manager**，再新开一个任务输入：
...
```text
~/.codex/skills/manage-external-memory/SKILL.md
```

不要多套一层同名文件夹。确认后新开一个 Codex 任务。
